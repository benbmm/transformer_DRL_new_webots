...
class HexapodBalanceEnv(Supervisor, gym.Env):
    ...
    def _calculate_reward(self):
        ...
        # 邊界懲罰
        if not (-0.2 <= x <= 0.2 and -0.18 <= y <= 0.18):
            p_s = -1
        
        total_reward = r_s + p_s
        
        return total_reward, r_s, p_s
    ...
    def _apply_actions(self, rl_corrections):
        ...
        for leg_idx in range(1, 7):
            ...
            for joint_idx in range(1, 4):  # 3個關節
                ...
                # 儲存處理過的訊號（用於踝關節引用）
                processed_signals[leg_idx][joint_idx] = motor_angle  # 注意：這裡儲存的是加RL修正前的訊號
                
                # 發送到馬達
                try:
                    if self.current_step >= self.control_start_step:
                        self.motors[leg_idx][joint_idx].setPosition(final_motor_angle)
                    else:
                        # 初始階段的處理（不加RL修正）
    ...
    def reset(self):
        ...
        # 初始化序列
        self._update_sequences(initial_state, None, initial_reward)
    ...
    def step(self, action):
        """執行一步動作"""
        # 儲存當前RL修正量供踝關節使用
        self._current_rl_corrections = action
        ...
        # 更新序列
        self._update_sequences(new_state, action, reward)
